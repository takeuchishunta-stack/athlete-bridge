...
if (!existsSync(ARTICLES)) {
  ...
  if (articles !== undefined) {
    if (!Array.isArray(articles)) {
      ...
      const required = ['id', 'category', 'date', 'title', 'company', 'personRole', 'personName', 'excerpt', 'blocks'];
      const seen = new Map();
      ...
      articles.forEach((article, i) => {
        ...
        if (Array.isArray(article.blocks)) {
          if (article.blocks.length === 0) errors.push(`${label}: blocks が空です（本文がありません）`);
          article.blocks.forEach((block, j) => {
            if (typeof block !== 'object' || block === null) {
              errors.push(`${label}: blocks[${j}] がオブジェクトではありません`);
              return;
            }
            if (block.type && !BLOCK_TYPES.has(block.type)) {
              // main.js は未知の type を p として描画してしまうので気付けるようにする
              warnings.push(`${label}: blocks[${j}] の type "${block.type}" は未対応です（段落として表示されます）`);
            }
